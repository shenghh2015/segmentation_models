...
model_name = 'vnet-set-{}-dim-{}-dep-{}-val_dim-{}-val_dep-{}-bz-{}-lr-{}-ep-{}-decay-{}-scale-{}'.format(dataset,
							dim, dep, val_dep, val_dim, val_dep, BATCH_SIZE, LR, EPOCHS, decay, scale)
model_folder = '/data/3d_models/vnet-{}/{}'.format(dataset, model_name) if args.docker else './3d_models/vnet-{}/{}'.format(dataset, model_name)
generate_folder(model_folder)

## network architecture
model = vnet(input_shape = (1, None, None, None))
...
class HistoryPrintCallback(tf.keras.callbacks.Callback):
		...
		def on_epoch_end(self, epoch, logs=None):
				...
				if epoch%10 == 0:
						...
						save_images(model_folder+'/epoch-{}_pr.png'.format(epoch), fl_prs*255./scale)
