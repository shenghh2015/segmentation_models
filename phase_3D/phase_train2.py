...
class Dataset:
    ...
    def __getitem__(self, i):
        image = np.load(self.images_fps[i]).transpose([2,0,1]);#print(image.shape)
        mask  = np.load(self.masks_fps[i]).transpose([2,0,1])
        mask = mask/255.*self.scale    

				# random cropping: (z, x, y)
        hsizes = self.hsizes
        vol_shp = mask.shape
        x_bd = [hsizes[0],vol_shp[1]-hsizes[0]]
        y_bd = [hsizes[1],vol_shp[2]-hsizes[1]]
        xc = random.randint(x_bd[0], x_bd[1])
        yc = random.randint(y_bd[0], y_bd[1])
        X_sample = image[:,xc-hsizes[0]:xc+hsizes[0],yc-hsizes[1]:yc+hsizes[1]]
        Y_sample = mask[:,xc-hsizes[0]:xc+hsizes[0],yc-hsizes[1]:yc+hsizes[1]]
...
model = unet_model_3d(input_shape = (1, None, None, None), n_base_filters=16, pool_size=(2, 2, 1), depth = 4)

## testing training dataset
train_dataset = Dataset(x_train_dir, y_train_dir, scale = 100, hsizes = [256, 256])
valid_dataset = Dataset(x_valid_dir, y_valid_dir, scale = 100, hsizes = [256, 256])
...
model_name = '3D_phase_d8_filter16'
...
test_dataset = Dataset(x_test_dir, y_test_dir, scale = 100, hsizes = [256, 256])
