...
## screen the fl1
model_name = 'EXP-net-{}-bone-{}-pre-{}-epoch-{}-batch-{}-lr-{}-dim-{}-train-{}-rot-{}-set-{}-subset-{}-loss-{}-act-{}-scale-{}-decay-{}-delta-{}-chi-{}-cho-{}-chf-{}-bselect-{}-extra-{}'.format(args.net_type, args.backbone, args.pre_train,\
...
train_dim = args.dim
val_dim = 1792
dataset, subset = args.dataset, args.subset

x_train_dir = DATA_DIR+'/{}/phase'.format(subset)
y1_train_dir = DATA_DIR+'/{}/fl1'.format(subset)
y2_train_dir = DATA_DIR+'/{}/fl2'.format(subset)

if dataset == 'spheroids_v6':
		x_valid_dir =DATA_DIR+'/val/phase'
		y1_valid_dir =DATA_DIR+'/val/fl1' 
		y2_valid_dir =DATA_DIR+'/val/fl2'
else:
		x_valid_dir =DATA_DIR+'/test/phase'
		y1_valid_dir =DATA_DIR+'/test/fl1'
		y2_valid_dir =DATA_DIR+'/test/fl2'
...
class HistoryPrintCallback(tf.keras.callbacks.Callback):
		...
		def on_epoch_end(self, epoch, logs=None):
				...
				if epoch%5 == 0:
						...
						if epoch == 0:
								save_images(model_folder+'/fl_gt.png'.format(epoch), gt_vols/args.scale*255)
						save_images(model_folder+'/pr_{}.png'.format(epoch), pr_vols/args.scale*255)
