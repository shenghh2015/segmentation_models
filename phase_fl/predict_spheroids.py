...
model_pool = read_txt('./spheroids_list.txt')
...
DATA_DIR = '/data/datasets/spheroids_infer_x{}'.format(dataset[-1])
if dataset == 'neuron_wbx1' or dataset == 'spheroids_dataset_x1':
    val_dim = 1760  # 1744
    offset = 8

volume_fns = [fn for fn in os.listdir(DATA_DIR) if 'output' in fn]

train_file = 'train_sample_list.txt' if dataset == 'neuron_wbx1' else 'train_list.txt'
test_file = 'test_sample_list.txt' if dataset == 'neuron_wbx1' else 'test_list.txt'

train_fns = read_txt(DATA_DIR+'/{}'.format(train_file))  
test_fns = read_txt(DATA_DIR+'/{}'.format(test_file))
print(test_fns)
