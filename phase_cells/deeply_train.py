...
model = net_func(BACKBONE, encoder_weights=encoder_weights, encoder_freeze=False, classes=n_classes, activation=activation)
...
model.compile(optimizer=optim, loss=total_loss, metrics = metrics, loss_weights=[0.5, 0.5, 0.5, 0.5, 1])
...
assert train_dataloader[0][1][-1].shape == (BATCH_SIZE, args.dim, args.dim, n_classes)
...
pr_masks = model.predict(test_dataloader)[-1]; pr_maps = np.argmax(pr_masks,axis=-1)
gt_masks = []
